...
def build_bcm(model, bcm, rule):
    ...
    pre_filtered = model.build(Lowpass(bcm.pre_tau), input=pre_activities)
    post_activities = model.sig[get_post_ens(conn).neurons]['out']
    post_filtered = model.build(Lowpass(bcm.post_tau), input=post_activities)
    theta = model.build(Lowpass(bcm.theta_tau), input=post_filtered)
...
def build_oja(model, oja, rule):
    ...
    pre_activities = model.sig[get_pre_ens(conn).neurons]['out']
    post_activities = model.sig[get_post_ens(conn).neurons]['out']
    pre_filtered = model.build(Lowpass(oja.pre_tau), input=pre_activities)
    post_filtered = model.build(Lowpass(oja.post_tau), input=post_activities)
...
def build_pes(model, pes, rule):
    ...
    acts = model.build(Lowpass(pes.pre_tau),
                       input=model.sig[conn.pre_obj]['out'])
