...
def test_voja_encoders(Simulator, nl_nodirect, seed, plt):
    ...
    np.random.seed(seed)  # for UniformHyersphere
    encoders = np.append(
        UniformHypersphere(surface=True).sample(n_change, len(learned_vector)),
        [-learned_vector]*(n - n_change), axis=0)
